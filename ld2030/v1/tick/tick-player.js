...
const PLAYER = {
  ...
  START_AP: PLAYER_CONFIG.baseAp ?? 3,
  MAX_AP: PLAYER_CONFIG.maxAp ?? PLAYER_CONFIG.baseAp ?? 3,
};
...
function makePlayerTicker({ reader, writer }) {
  ...
  const maxAp = Number.isFinite(PLAYER.MAX_AP)
    ? Number(PLAYER.MAX_AP)
    : Number(PLAYER.START_AP ?? 3);
  const hpRegen = Number(TICK?.PLAYER?.HP_REGEN_PER_TICK ?? 1);
  const maxHp = Number.isFinite(PLAYER.MAX_HP)
    ? Number(PLAYER.MAX_HP)
    : Number(PLAYER.START_HP ?? 100);
  ...
  async function tickPlayer({ gameId = 'lockdown2030', now } = {}) {
    ...
    snap.forEach((doc) => {
      ...
      const curAp = Number.isFinite(curApRaw)
        ? Number(curApRaw)
        : Number(PLAYER?.START_AP ?? 0);
      ...
      const curHp = Number.isFinite(curHpRaw)
        ? Number(curHpRaw)
        : Number(PLAYER?.START_HP ?? 0);
